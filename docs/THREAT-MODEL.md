...
| A20 | Machine-local rendered state drifts from signed templates, or a `~/.codex` symlink dangles/repoints, outside the signed skill dir | A skill bin (e.g. `codex-docs-drift-scout`) renders machine-specific files from signed `.tpl` templates, plants a `~/.codex` symlink, and records a machine-local render index (`$AUTOVAULT_STORAGE_PATH/render-index.json`, a sibling of `rendered/`). The installer fails closed when an existing index is corrupt or not v1, constrains render roots and symlinks, stages and TOML-validates rendered files, then atomically replaces the index and link. `autovault doctor [--json]` re-verifies signed templates, detects staleness and rendered-file drift/deletion, confirms link targets, and on full runs scans both recorded roots and the active `${CODEX_HOME:-~/.codex}/automations` root for orphan links even when the index is absent. **Ceiling:** the index lives outside `rendered/` so `rm -rf rendered` surfaces as errors instead of vanishing; this defends against accidental drift and weaker-privilege tampering only. A same-UID writer can forge the index or re-sign templates — out of scope, identical to the "treat storage-root write access as full vault compromise" accepted risk below. |
