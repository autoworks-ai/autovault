...
| A11b | Finder/editor metadata creates false tamper warnings | The open-set integrity walk ignores regular benign metadata artifacts (`.DS_Store`, `Thumbs.db`, `desktop.ini`, AppleDouble `._*`) and `autovault doctor --clean` removes only those artifacts. Symlinks, special files, unknown hidden files, unsigned helpers, and changed signed files remain integrity failures. |
...
| A20 | Machine-local rendered state drifts from signed templates, or a `~/.codex` symlink dangles/repoints, outside the signed skill dir | A skill bin (e.g. `codex-docs-drift-scout`) renders machine-specific files from signed `.tpl` templates, plants a `~/.codex` symlink, and records a machine-local render index (`$AUTOVAULT_STORAGE_PATH/render-index.json`, a sibling of `rendered/`). `autovault doctor [--json]` is the non-interactive (no TTY gate) render-fidelity check: it re-verifies the templates via `readVerifiedSkillResources` (open-set integrity walk under the storage lock), hash-compares them against the recorded render-time hash (staleness), hash-compares the on-disk rendered files (hand-edit/deletion), confirms the symlink targets the render root, and closed-set sweeps each managed link root for orphan symlinks with no backing entry. **Ceiling:** the index lives outside `rendered/` so `rm -rf rendered` surfaces as errors (missing file + dangling symlink) instead of vanishing; the check defends against accidental drift and weaker-privilege tampering only. A same-UID writer can forge the index or re-sign templates — out of scope, identical to the "treat storage-root write access as full vault compromise" accepted risk below. |
