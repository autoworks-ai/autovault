...
export async function runLinkCommand(args: string[]): Promise<void> {
  ...
  if (!target) {
    process.stderr.write("Usage: autovault link <slug|catalog-url|directory> [--json]\n");
    process.exit(1);
  }

  const { completeEnrollmentFromTarget, refreshEnrollment } = await import("../sync/local.js");
  let enrollment = await withSuppressedLogs(() => completeEnrollmentFromTarget(target));
...
function formatLinkResult(enrollment: EnrolledUpstream): string {
    ...
    [
      {
        ...
        status: enrollment.enrollment.status === "active" ? "ok" : pending ? "warn" : "error"
      },
      { label: "device", value: enrollment.enrollment.device_id, status: "muted" },
      { label: "key", value: `ed25519 ${fingerprint}`, status: "muted" },
      ...(pending
        ? [{ label: "next", value: `Admit this machine at ${cloudAdmitUrl()}`, status: "warn" as const }]
        : [])
    ],
    theme
  );
  ...
  return `${theme.style.yellow(theme.symbol.warn)} ${theme.style.bold(`Linked ${enrollment.name}`)} ${theme.style.dim("waiting for admit")}\n${rows}\n`;
}
