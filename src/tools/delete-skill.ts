...
export async function deleteSkill(input: DeleteSkillInput): Promise<Record<string, unknown>> {
  ...
  const transformRoot = path.join(loadConfig().storagePath, "transforms", name);
  await fs.rm(transformRoot, { recursive: true, force: true }).catch((error) => {
    log.warn("delete_skill.transform_cleanup_failed", { name, error: String(error) });
  });
  ...
  try {
    ...
    log.warn("delete_skill.profile_sync_failed", { name, error: String(error) });
    warnings.push(message);
    return { deleted, name, warnings };
  }
}
